...
$(document).ready(function() {
	...
	$("#dataForm").on("submit", function(event) {
		...
		inputsImage.find("img").each(function(i, o) {
			$(o).attr("src", "http://www.simumath.com/tools/drawn/"+$(o).attr("src"));
...
function showResults(title, content, type, time) {
	...
	if(type == "chart") {
		thisButtons.unshift({text: "Print",
			...
			click: function() {
				...
				if($("div.drawnResults-"+time+" input.TAB"+tabNum+"_0:checked").length > 0) {
					...
					window.open("http://www.simumath.com/tools/utils/printChart.php?values="+btoa(valuesArray.toString())+"&titles="+encodeURIComponent(titlesArray.toString())+"&time="+btoa(time), "_blank");
				}
				loadingIcon(false);
			}
		});
...
function fillOutput(resultData, time) {
	...
	if(typeof resultData[1] != 'undefined') {
		...
		$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last").clone().insertAfter("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last");
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last td:eq("+v+")").html('<input title="Show in Graph" id="TAB1_cb'+(v+1)+'" class="TAB1_0" type="checkbox" points="['+graphArrayOne[1][v]+']" pointsTwo="['+graphArrayTwo[1][v]+']" />');
		}
	...
	if(typeof resultData[2] != 'undefined') {
		...
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB2_0.NSW.M_"+thisM+":last td:eq("+v+")").html('<input title="Show in Graph" id="TAB2_cb'+(v+1)+'" class="TAB2_0" type="checkbox" points="['+graphArrayOne[2][v]+']" pointsTwo="['+graphArrayTwo[2][v]+']" />');
		}
	...
	if(typeof resultData[3] != 'undefined') {
		...
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB3_0.NSW.M_"+thisM+":last td:eq("+v+")").html('<input title="Show in Graph" id="TAB3_cb'+(v+1)+'" class="TAB3_0" type="checkbox" points="['+graphArrayOne[3][v]+']" pointsTwo="['+graphArrayTwo[3][v]+']" />');
		}
...
function createPDF(obj) {
	...
	resultsWindow = window.open("http://www.simumath.com/tools/utils/pdf.php?c="+btoa(page), 'drawnPDF', 'width=1024, height=800');
...
function showGraph(tabNum, time) {
	...
	if($("div.drawnResults-"+time+" input.TAB"+tabNum+"_0:checked").length > 0) {
		...
		.done(function(data) {
			showResults("Graph", data, "chart", time);
...
function exportData() {
	var thisFile = window.prompt("Please name your file", "inputData");
	if (thisFile != null) {
